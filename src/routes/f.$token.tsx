...
import { Leaf, Send, CheckCircle2, AlertTriangle } from "lucide-react";
...
export const Route = createFileRoute("/f/$token")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Formulário exclusivo de feedback do protótipo Nutr.IA." },
    ],
  }),
  component: FormPage,
});

type Respondent = { id: string; name: string; token: string };
...
function FormPage() {
  ...
  if (loading) {
    return (
      <div className="container mx-auto max-w-2xl px-4 py-20 text-center text-muted-foreground">
        Carregando…
      </div>
    );
  }
        ...
        <Card className="border-destructive/30">
          <CardContent className="flex flex-col items-center gap-4 py-12 text-center">
            ...
            <p className="text-muted-foreground">
              Este link de feedback não existe ou foi removido. Verifique se o endereço está correto.
            </p>
          </CardContent>
        </Card>
      </div>
        ...
        <Card className="border-primary/30">
          <CardContent className="flex flex-col items-center gap-4 py-12 text-center">
            ...
            <h2 className="text-2xl font-bold">
              {submitted ? "Feedback registrado!" : "Feedback já enviado"}
            </h2>
            <p className="text-muted-foreground">
              {submitted
                ? `Obrigado, ${respondent.name}! Sua contribuição é fundamental para a evolução do Nutr.IA.`
                : `Olá, ${respondent.name}. Este link já foi utilizado e permite apenas um envio.`}
            </p>
          </CardContent>
        </Card>
      </div>
    ...
    <div className="container mx-auto max-w-3xl px-4 py-10">
      <header className="mb-10 text-center">
        ...
        <p className="mx-auto mt-4 max-w-xl text-muted-foreground">
          Olá, <strong className="text-foreground">{respondent.name}</strong>! Obrigado por participar dos testes do
          Nutr.IA. Seu feedback é fundamental para validar nossa pesquisa de Engenharia de Software. Este formulário
          leva menos de 3 minutos e pode ser enviado apenas uma vez.
        </p>
      </header>
